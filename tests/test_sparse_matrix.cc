...
RC_GTEST_PROP(SparseMatrix, SparseMatrixIndexesLikeDense, (const int n, const std::array<std::array<double, 10>, 10> mat)) {
    ...
    for (const auto& row: mat) {
        ...
        std::copy_if(row.begin(), row.end(), std::back_inserter(nonzeros), [](double x) { return x != 0.0; });
