...
void CplexSolver::set_parameter(const Param param, const int value) {
  ...
  detail::cplex_function_checked(CPXsetintparam, env_.get(),
                                 int_param_dict_.at(param), value);
}
...
Status CplexSolver::solve() { throw NotImplementedError(); }

Status CplexSolver::solution_status() const { throw NotImplementedError(); }

const ILinearProgramHandle& CplexSolver::linear_program() const {
  return lp_handle_;
}

ILinearProgramHandle& CplexSolver::linear_program() { return lp_handle_; }
